...
public class CTHDDAOo {
    ...
    public List<SanPham> banChay (){
        Connection cn = con.getJDBCConnection();
        List<SanPham> list = new ArrayList<>();
        String sql = "select * --sản phẩm bán chạy nhất\n" +
                    "from tblSanPham\n" +
                    "where id in(\n" +
                    "select idSP\n" +
                    "from tblChiTietDonHang as c , tblDonHang as d\n" +
    ...
    public List<SanPham> banChay_IdLoai (String idLoai){
        ...
        String sql = "select *\n" +
                    ...
                    "where id in ( \n" +
                    "select idSP\n" +
                    "from tblChiTietDonHang as c, tblDonHang as d , tblSanPham as s\n" +
                    "where c.idDH = d.maDH and c.idSP = s.id and d.thanhToan = 'r' and s.DMNo = ?\n" +
                    "group by idSP  \n" +
                    "having sum(SLMua) = ( select top 1 sum(SLMua) soLuong  from tblChiTietDonHang as c, tblSanPham as s , tblDonHang as do \n" +
                    "where c.idSP = s.id and s.DMNo = ? and c.idDH = do.maDH and do.thanhToan = 'r'  \n" +
                    "group by c.idSP, s.id order by soLuong desc) \n" +
                    ")";
        try {
            PreparedStatement pstm = cn.prepareStatement(sql);
    ...
    public List<SanPham> banChay_IdLoai_Ngay (String idLoai, Date bd, Date kt){
        ...
        String sql = "select *\n" +
                    ...
                    "where id in (\n" +
                    "select idSP \n" +
                    "from tblChiTietDonHang as c, tblDonHang as h, tblSanPham as s\n" +
                    "where c.idDH=h.maDH   and c.idSP = s.id and h.thanhToan = 'r' and convert(date,ngayTaoDH,103) between convert(date,?,103) and CONVERT(date,?,103) and s.DMNo =?\n" +
                    "group by idSP \n" +
                    "having sum(SLMua) = ( select top 1 sum(SLMua) soLuong  from tblChiTietDonHang as c, tblDonHang as h,tblSanPham as s\n" +
                    "where c.idDH=h.maDH and c.idSP = s.id and s.DMNo = ? and \n" +
